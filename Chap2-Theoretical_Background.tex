...
\section{GNC System}
    ...
    % After the determination of a path, the guidance system make the path available to the control system.
    
    \subsection{Guidance System}


    % In an autonomous approach, most \acp{USV} guidance systems are responsible for planning the path that will be traveled by the \ac{USV}. 
    % % For determination of a path, the guidance system uses available information about static and dynamic obstacles gathered by the navigation system. 
    % For determination of a path, the guidance system uses the information gathered by the navigation system related to the environment and to the \ac{OV}'s state. In general, trajectory generation shall be in accordance with the \ac{USV} mission and marine protocols, such as being in accordance with the \ac{COLREGS} or respect \ac{TSS} definitions (\ac{TSS} are similar to traffic ground lines that must be respected by vessels on navigation at sea.) Also, information about vehicle capability (e.g. maximum speed and power consumption)
    % %AMA dar uma exemplo de vehicle capability
    % %DJ: Done
    % and environmental conditions (e.g. wind, wave, and current disturbance)
    % %AMA dar exemplos de environmental conditions que podem ser relevantes
    % %DJ: Done
    % may be required to determine suitable trajectories.
    ...
    %AMA me parece q deveria ter pelo menos uma ref para essas heuristicas. um survey, por exemplo.
    %DJ: Done

    %%%%%%%%%%%%%%%%%%
    % Grammarly: 100/100
    %%%%%%%%%%%%%%%%%%
    % In Figure \ref{fig:chap2_globallocalpath} we illustrate the difference between global and local planning in our context. For a situation where our own vessel (\acs{OV}), is not capable of
    % %AMA capable of + verbo-ing ..... capable of detecting. verificar se existem outras ocorrencias
    % detecting some vessel in the far-field, the global planner could define a route above a location occupied by the encountering vessel (\acs{EV}). In this situation the local planner should react when \ac{EV} appears in the near-field and define an avoidance behavior. Both global and local planners usually define the \ac{USV} trajectory considering static and dynamic obstacles.
    ...
    % Grammarly: 100/100
    %%%%%%%%%%%%%%%%%%
    %AMA me parece q essa definicao deveria vir antes. deixo a teu criterio
    %DJ: modifiquei o texto acima para que esse ultimo parágrafo fique numa sequência lógica.
    ...
    % The navigation system is responsible for the determination of the current state of the USV (e.g., position, velocity, orientation, and acceleration) and environment perception (e.g., static and dynamic obstacles, other vessels, wind speed, and water current).
    % %AMA esse tipo de exemplo eh bem importante. acho q se vc colocar la na 1a vez q vc menciona esses termos ja eh o suficiente. por outro lado, melhor sobrar do que faltar. de qualquer forma. eh importante ter PELO MENOS nas 1a mencoes
    % %DJ: Adicionei nas 1as menções
    % For current state estimation, the navigation system can use GPS, \ac{IMU}, and compasses.
    % %AMA barometer eh usado em USV ?!?! certeza ?
    % %DJ: removed
    % Regarding environment perception, static information about the environment is extracted from nautical charts and topographic maps. On the other hand, the dynamic information about the environment is acquired in real-time sensing by the navigation system through the usage of cameras, \ac{LIDAR}, radar, and \ac{AIS}. \ac{AIS} allows automated message exchange between vessel, facilitating the identification of other vessels location, velocity, course, path, dimension, and type. 
