...
\label{chap:5_Simulation_And_Results}
    ...
    % We developed 12 scenarios to evaluate the system focusing on comparison with simulations presented in reference studies, \ie{} Agrawal \etal{} ~\cite{Agrawal2015COLREGS} and Huang \etal{} ~\cite{Huang2019Generalized}. We chose both studies for their quality (\ie{} h-index > 50) and similarity with our work. Agrawal \etal{} present our problem-solving inspiration but unfortunately, they do not present too much information about their system evaluation, so we based our tests and system evaluation on Huang \etal{} as they explicitly present their simulation scenarios characterization, results, and simulate a \ac{USV} similar to ours in respect to the dimensions.
    ...
    % We developed 12 scenarios to evaluate the system. We evaluate the behavior of our system considering the four main encounters described in the \ac{COLREGS} (head-on, crossing from the right, crossing from the left, and overtaking). 
    % focusing on comparison with simulations presented in reference studies, \ie{} Agrawal \etal{} ~\cite{Agrawal2015COLREGS} and Huang \etal{} ~\cite{Huang2019Generalized}. We chose both studies for their quality (\ie{} h-index > 50) and similarity with our work. Agrawal \etal{} present our problem-solving inspiration but unfortunately, they do not present too much information about their system evaluation, so we based our tests and system evaluation on Huang \etal{} as they explicitly present their simulation scenarios characterization, results, and simulate a \ac{USV} similar to ours in respect to the dimensions.
...
%     \end{figure}
    ...
    \section{Experiments Results}
        ...
        % This happens due to an intrinsic characteristic of our A* method. In the beginning \ac{AV} appears in the limit of \ac{OV} local cost map in a conflicting position with local map A* goal. In this situation, the planning system searches for the nearest free position and define a path to it. After some time, \ac{AV} is no longer in a conflicting position, and the computational time reduces dramatically. Due to this behavior, our system presents a low average execution time around 0.129s when compared to the maximum computational time.
        ...
        % In this scenario the \ac{OV} achieved a maximum velocity of 0.43 m/s.
        ...
        % The final position of the \ac{AV} for "Wind case" is not show in the plot presented in Figure \ref{fig:plot_cr_w_vs_wind} due to data loss in collection process.\todo{Do collection again} 
        % In this scenario the \ac{OV} achieved a maximum velocity of 0.43 m/s
        
        % Comparando a trajetória do \ac{OV} para os casos com e seu vento, podemos observar que no caso sem vento o \ac{OV} se desloca mais para northeast than in the with wind case. This happens due the atraso de deslocamento do \ac{AV}. O efeito
        % Comparing the \ac{OV}'s trajectory in both cases, we can see that in the "no wind case" the \ac{OV} goes to northwest more than in the "wind case". This occurs due to \ac{AV}'s behavior. The \ac{AV}  confront wastes more time to achieve its goal.   In Figure \ref{fig:plot_cr_w_vs_wind} we want to highlight to points. The first one regarding \ac{OV}'s trajectory comparison between
        ...
        % In Figure \ref{fig:plot_r_w_vs_wind_CT} we show the computational time measured in seconds over time for wind and no wind cases. We observe that the computational time remains stable and and similar to the previously presented cases. The computational time does not seem to be related to the imposed wind influence for this simulation.
        ...
        %% Grammarlly: 99/100
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        Figure \ref{fig:plot_cl_w_vs_wind_CT} shows the computational time measured in seconds over time for wind and no wind cases. We observe that the computational time remains stable and and similar to the previously presented cases. The computational time does not seem to be related to the imposed wind influence for this simulation. \todo{missing discussion}
        \begin{figure}[H]
            \centering
        ...
        %% Grammarlly: 99/100
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        Figure \ref{fig:plot_ov_w_vs_wind_CT} shows the computational time measured in seconds over time for wind and no wind cases. We observe that the computational time remains stable and and similar to the previously presented cases. The computational time does not seem to be related to the imposed wind influence for this simulation. \todo{missing discussion}
        \begin{figure}[H]
            \centering
        ...
        %AMA ZOOOONA. o texto diz q eh overtaking e a legenda diz q eh crossin right. olhando a figura parece que a figura esta certa e o texto errado.mas esse tipo de erro da um no no leitor pois ele omeca a duvidar se entendeu alguma coisa.
        
        % In Figure \ref{fig:plot_overtaking_w_vs_wind} we show the comparison between trajectories with and without \ac{ATC} and in Figure \ref{fig:plot_overtaking_w_vs_wind_CT} we can see computational time for each case. In an overtaking encounter, \ac{OV} must keep moving ahead, avoiding to generate any other crossing situations while overtaking. In our simulation, \ac{OV} avoids going to the front of \ac{AV} until it gets near the goal.
        ...
        %         \includegraphics[width=\textwidth]{figs/Chap5/plot_cl_w_vs_wind.pdf}
        %         \caption{Trajectory}
        %         \label{fig:plot_overtaking_w_vs_wind}
        %     \end{subfigure}
        %     \begin{subfigure}[b]{0.49\textwidth}
        %         \centering
        %         \includesvg[width=\textwidth]{figs/Chap5/plot_cl_w_vs_wind_CT.svg}
        %         \caption{Computation Time}
        %         \label{fig:plot_overtaking_w_vs_wind_CT}
        %     \end{subfigure}
        
        % \caption{Overtaking Encounter Scenario. Comparing \ac{ATC} versus no \ac{ATC} cases and, with and Wind vs no Wind cases.}
        % \label{fig:plots_ov}
        % \end{figure}
        ...
        %% Grammarlly: 100/100
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % In Figure \ref{fig:plots_ov}, we present the behavior of our system when \ac{OV} encounters another vessel ahead and should a overtaking.
        
        .  %AMA a legenda diz overtaking ... confere os nomes e mantem a coerencia
        %AMA isso ainda nao foi corrigido !!!
        
        % In Figure \ref{fig:plot_ov_w_vs_wo} we show the comparison between trajectories with and without \ac{ATC} and in Figure \ref{fig:plot_ov_w_vs_wo} we can see computational time for each case. As we can see, once again, our method implies COLREGS compliance when avoiding a collision. In Figure \ref{fig:plot_overtaking_w_vs_wind} we show the comparison between trajectories with and without wind influence and in Figure \ref{fig:plot_overtaking_w_vs_wind_CT} we can see computational time for each case. 
        % \begin{figure}[H]
        % \centering
